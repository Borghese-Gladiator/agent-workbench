...
This is explicitly a single-developer-machine tool (a founding constraint —
see `AGENTS.md` › "Things NOT to do"). Postgres would mean a server process to install, configure, start,
...
- **A vector database for repository memory retrieval.** Explicitly a
  non-goal — FTS5 plus structural signals (exact path, symbol
