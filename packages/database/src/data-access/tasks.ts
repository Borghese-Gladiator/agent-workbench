...
import type { TaskPhase, RunCondition, DeliveryState, TaskSize, HumanGateReason } from '@awb/domain';
...
/**
 * Recomputes the durable `task_summary` row for a task from current SQLite state. This is the fix for
 * the walkthrough's stale-list bug: the daemon calls it on EVERY workflow transition it persists
 * (task upsert, run-state snapshot, observability), so the list/board/approval read model tracks the
 * workflow even after a task parks awaiting-human (when the bare `tasks` row stopped moving).
 *
...
export function refreshTaskSummary(db: DrizzleDb, taskId: string, context: TaskSummaryContext = {}): void {
  ...
  const pendingGateReason =
    context.pendingGateReason !== undefined ? context.pendingGateReason : (prior?.pendingGateReason as HumanGateReason | null | undefined) ?? null;
  const candidateSha =
...
export function listTaskSummaries(db: DrizzleDb): TaskSummaryWithRepository[] {
  return db
    .select({
...
export function backfillTaskSummaries(db: DrizzleDb): number {
  const allTaskIds = db.select({ id: tasks.id }).from(tasks).all().map((r) => r.id);
  const summarized = new Set(db.select({ id: taskSummary.taskId }).from(taskSummary).all().map((r) => r.id));
  let created = 0;
