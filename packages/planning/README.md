...
builder's per-slice implementation loop.
...
- `planner-critic-loop.ts` — `runPlannerCriticLoop()`: the bounded
  planner↔critic exchange. The planner and critic are
...
- `builder-loop.ts` — `runSliceLoop()`: one plan slice's bounded builder
  loop, using `@awb/workflow`'s failure-fingerprinting
...
- Persist contracts/plans to SQLite — that's the daemon's job; this package
  returns fully-formed domain objects.
