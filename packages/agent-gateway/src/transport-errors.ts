 ...
 * dropped mid-response) rather than a logic/engineering failure (TASK-32). A transport drop is
 * resumable: the retry should continue the prior session (via its persisted resume token) instead of
 * treating the failure as a reason to cold-restart or block. Live run 5a513429 surfaced
 * `API Error: Connection closed mid-response` from the Claude Agent SDK's `readMessages`; ~47 min was
 * wasted re-doing work because each Temporal retry cold-started. Matching this string is what lets the
 * retry resume.
 */
