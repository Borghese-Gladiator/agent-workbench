...
/**
 * The `discoverRepository` Activity behind `RepositoryDiscoveryWorkflow` (spec §9/§15, TASK-26).
 * Discovery writes snapshot rows, so — like all run-state writes — it goes through the daemon (the
 * single application writer, spec §8): the activity calls the daemon's refresh endpoint, which runs
